...
public class StairsStage implements Stage {
	...
	public void execute(ProcessingContext context, ProgressListener listener) {
		...
		stairsLoop: for (int i = 0; i < w * h / 100; i++) {
			...
			count++;
			if (count == LayerRatio.GROUND.MAX_STAIRS_COUNT)
				break;
		}
		input.mapData()[0]=map;
		context.advanceStage();
		listener.onProgressUpdate(context.getCurrentStageIndex(), context.getTotalStages(),"GROUND", getName(), "Generated stairs");
	}
